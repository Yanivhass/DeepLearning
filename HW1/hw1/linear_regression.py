...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # ====== YOUR CODE: ======
        n = X.shape[0]
        reg = n*self.reg_lambda*np.identity(X.shape[1])
        reg[0, 0] = 0  # no regularization for bias
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    corrs = df.corr()
    cor = corrs[target_feature]
    cor = cor.drop(labels=target_feature)
    cor = cor.abs()
    cor = cor.sort_values(ascending=False)
    cor = cor[0:n]
    top_n_features = cor.keys()
    top_n_corr = corrs[target_feature].loc[cor.keys()].values
    # ========================
...
def cv_best_hyperparams(
    ...
    for train_index, test_index in KFold.split(X):
        for degree in degree_range:
            for reg in lambda_range:
                # define folds
                X_train, X_test = X[train_index], X[test_index]
                y_train, y_test = y[train_index], y[test_index]
                # set hyper params
                hp['linearregressor__reg_lambda'] = reg
                hp['bostonfeaturestransformer__degree'] = degree
                model.set_params(**hp)
                # fit & predict
