...
def char_maps(text: str):
    ...
    char_to_idx = {char: index for char, index in zip(chars, indices)}
    idx_to_char = {index: char for char, index in zip(chars, indices)}

    # number_of_unique_char = set(text)
    # char_to_idx={}
    # idx_to_char={}
    # index =0
    # for iter in text:
    #     if((iter not in char_to_idx)):
    #         char_to_idx[iter] = index
    #         idx_to_char[index]=iter
    #         index+=1
    # ========================
...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    for c in range(0, len(text)):
        arr.append(char_to_idx[text[c]])
    res = torch.tensor(arr)
    result = torch.nn.functional.one_hot(res, num_classes=len(char_to_idx))
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    result = torch.nonzero(embedded_text, as_tuple=True)[1]
    chars = ''
    for c in result:
        chars += idx_to_char[c.item()]
    result = chars
    # ========================
    return result
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    trim = (len(text)-1) % seq_len
    if trim > 0:
        embedded = embedded[:-trim]
    labels = embedded[1:].argmax(axis=1)
    embedded = embedded[:-1]
    samples = torch.stack(list(embedded.split(seq_len)), dim=0)
    labels = torch.stack(list(labels.split(seq_len)), dim=0)
    # ========================
