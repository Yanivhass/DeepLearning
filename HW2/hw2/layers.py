...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        self.dw += dout.T @ x 
        self.db += torch.sum(dout, dim=0).reshape_as(self.b)
        dx = dout @ self.w
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        loss = 0
        xy = torch.gather(x, dim=1, index=torch.unsqueeze(y,dim=1))
        loss = -xy + torch.unsqueeze(torch.log(torch.sum(torch.exp(x), dim=1)),dim=1)
    ...
    def backward(self, dout=1.0):
        ...
        xy = torch.gather(x, dim=1, index=y)
        a = torch.exp(x)
        b = torch.sum(torch.exp(x), dim=1)
        b = torch.unsqueeze(b, dim=1)
        s = a/b
        a = torch.zeros_like(s).scatter_(1, y, 1)
        dx = (s - a)/N
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode is True:
            prob = torch.rand(x.shape)
            drop = torch.bernoulli(torch.full_like(x, 1 - self.p))
            # drop = torch.where(prob > self.p, torch.ones_like(x), torch.zeros_like(x))
            # out = torch.where(drop == 1, x, torch.zeros_like(x))
            out = (drop*x)/(1-self.p)
            self.grad_cache["drop"] = drop

            # out = x * self.grad_cache["drop"]
        else:
            out = x
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        if self.training_mode is True:
            drop = self.grad_cache["drop"]
            dx = torch.where(drop == 1, dout, torch.zeros_like(dout))/(1-self.p)
            # dx = (drop*dout)
            # dx = dout
        else:
            dx = dout
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        for i in range(n):
            if i == 0:
                layers.append(Linear(in_features, hidden_features[i]))
                ...
                if activation == "sigmoid":
                    ...
                    if dropout != 0:
                        layers.append(Dropout(dropout))
            if i == n-1:
                layers.append(Linear(hidden_features[i], num_classes))
