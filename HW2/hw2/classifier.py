...
class ArgMaxClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        res = torch.argmax(y_proba, dim=1)
        return res
        # ========================
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        to_ret = torch.tensor( [0 if i <self.threshold else 1 for i in y_proba[:,self.positive_class]],dtype=torch.int).T
        return to_ret
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # ====== YOUR CODE: ======
    xs = torch.linspace(-4,4,steps = int(8/dx))
    ys = torch.linspace(-4,4,steps = int(8/dx))
    x1_grid, x2_grid = torch.meshgrid(xs,ys,indexing='xy')
    x1_grid_up = x1_grid.reshape(-1,1)
    x2_grid_up = x2_grid.reshape(-1,1)
    y_hat = classifier.classify(torch.hstack([x1_grid_up,x2_grid_up]))
    y_hat = y_hat.reshape(x2_grid.shape)
    # ========================
...
def select_roc_thresh(
    ...
    fpr, tpr, thresholds = roc_curve(y, y_hat[:,1].detach())
    a = torch.tensor(tpr-1)**2
    b = torch.tensor(fpr)**2
    dists = torch.sqrt(a + b)
    optimal_thresh_idx = torch.argmin(dists).int()
